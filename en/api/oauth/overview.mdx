...
<Note>
  If you only need public data — job listings, company profiles, or events — you do not need OAuth. Set up [App Key authentication](/en/api/authentication) instead and skip the OAuth flow entirely.
</Note>
...
See the [Auth Flow guide](/en/api/oauth/flow) for the complete implementation with code examples.
...
Request only the scopes your app genuinely needs — asking for unnecessary permissions reduces user trust and lowers approval rates. Available scopes are listed in the [Auth Flow guide](/en/api/oauth/flow).

## Next Steps

Now that you understand when and why to use OAuth, implement the full authorization flow in your application.

<Card title="Implement the Auth Flow" icon="arrow-right" href="/en/api/oauth/flow">
