---
title: "Rocketpunch API Error Codes and Troubleshooting Guide"
...
| `429` | `RATE_LIMIT_EXCEEDED` | Your app has exceeded its request quota for the current window. | Wait for the time indicated in the `Retry-After` header, then retry. See the [Rate Limits](/en/api/rate-limits) page for retry strategies. |
...
If you receive a `401` when using a Bearer token, the token may have expired. Use your `refresh_token` to obtain a new access token. See the [Auth Flow guide](/en/api/oauth/flow#token-refresh) for the refresh flow.
